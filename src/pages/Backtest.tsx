...
const strategyPresets: Record<string, { return: number; vol: number; asset: string; rebalance: string }> = {
  ...
  'Foreign Exchange': { return: 6, vol: 9, asset: 'EURUSD', rebalance: 'Weekly' },
...
export function Backtest() {
  ...
  // Apply strategy preset when strategy changes
  useEffect(() => {
    ...
    setRebalance(preset.rebalance)
  }, [selectedStrategyId])

  const run = () => {
    setRunning(true)
    setTimeout(() => {
      setResult(runBacktest(capital, years, annualReturn / 100, annualVol / 100))
      setRunning(false)
    }, 400)
  }
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-text">Run strategy-aware synthetic simulations across assets and timeframes.</p>
        </div>
      ...
      <div className="rounded-xl border border-accent-warning/30 bg-accent-warning/10 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-accent-warning">Synthetic simulation only</p>
            <p className="text-xs text-text">
              No live market data feed is connected. Returns are generated via Monte Carlo using the strategy’s
              assumed annual return and volatility. To run historical backtests, connect a data provider
              (Polygon, Alpaca, Yahoo Finance, Bloomberg, etc.) via the API keys in Settings.
            </p>
          </div>
        </div>
      </div>
              ...
              <Card>
                <CardHeader>
                  <div>
                    ...
                    <CardSubtitle>
                      {selectedStrategy.name} on {asset} · {years}Y · {rebalance} rebalance
                    </CardSubtitle>
                  </div>
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                  ...
                  <div className="rounded-lg border border-border bg-bg-card p-3">
                    ...
                    <div className="font-medium text-text-heading">Monte Carlo</div>
                  </div>
                </div>
              </Card>
              ...
              <Card>
                <CardHeader>
                  <div>
                    ...
                    <CardSubtitle>
                      Synthetic {years}-year performance for {selectedStrategy.name}
                    </CardSubtitle>
                  </div>
